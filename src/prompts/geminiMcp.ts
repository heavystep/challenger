/**
 * MCP Command Prompt @heavystep/prompts
 */
const geminiMcpPrompt = ({ url, act, geminiFunctions }: { url: string, act: string, geminiFunctions: any[] }) => `
...
<Tools>
${geminiFunctions.map(fn => `- ${fn.name}: ${fn.description}`).join('\n')}
</Tools>
...
export default geminiMcpPrompt;